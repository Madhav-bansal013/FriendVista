...
const PostCard = ({post, user, deletePost, likePost}) => {
  ...
  const getComments = async (postId) => {
    setLoading(true);
    const filteredComments = postComments.filter(comment => comment.postId === postId);
  ...
  return (
    <div className='bg-primary mb-2 p-4 rounded-lg'>
      ...
      <div className='mt-4 flex justify-between items-center px-3 py-2 text-ascent-2 text-base border-t border-[#66666645]'>
        ...
        <p className='flex items-center gap-2 text-base cursor-pointer'
        onClick={() => {
          const newShowComments = showComments === post?._id ? null : post?._id;
        ...
        </p>
          {
            user?._id === post?.userId?._id && (
              <div className='flex items-center text-base text-ascent-2 cursor-pointer gap-1'
                  ...
                  comments.map((comment) => (
                    <div className='w-full py-2' key={comment?._id}>
                      <div className='flex items-center gap-3 mb-1'>
                        ...
                        <div>
                          <Link to={"/profile/" + comment?.userId?._id }>
                            <p className='text-sm text-ascent-2'>{comment?.userId?.firstName} {comment?.userId?.lastName}  </p>
                          </Link>
                          <span className='text-sm text-ascent-2'>
